...
def send_mail(to, template, context):
    html_content = render_to_string(f'blog/mail/{template}.html', context)
    txt_context = render_to_string(f'blog/mail/{template}.txt', context)
...
def send_act_email(request, email, code):
    context = {
        ...
        'uri': request.build_absolute_uri(reverse('blog:activate', kwargs={'code': code})),
    }
...
def send_reset_password_email(request, email, token, uid):
    context = {
        ...
        'uri': request.build_absolute_uri(
            reverse('blog:restore_password_confirm', kwargs={'uidb64': uid, 'token': token})),
    }
...
def send_act_change_email(request, email, code):
    context = {
        ...
        'uri': request.build_absolute_uri(reverse('blog:change_email_activation', kwargs={'code': code})),
    }
